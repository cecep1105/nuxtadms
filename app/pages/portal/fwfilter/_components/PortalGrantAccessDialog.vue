 ...
 * non-staff). Aksi Grant Access ITU SENDIRI (POST
 * /firewall/grant-access/) TETAP endpoint yang SAMA dgn staff
 * (permission-nya SUDAH diperluas trima izin portal juga).
 */
...
async function handleSearch() {
  ...
  searchError.value = null
  try {
    const data = await request<{ results: MikrotikDhcpLease[] }>(`/netmgmt/portal/dhcp-lease/?_q=${encodeURIComponent(ipAddress.value)}&_search_fields=address&_limit=1`)
