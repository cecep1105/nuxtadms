...
import { Loader2, Search, User, Users, CheckCircle2, ArrowLeft, Wand2 } from "@lucide/vue"
import type { IDCardType, IDCardTemplate, IDCardHolder, IDCardPhotoCandidate, IDCardDetail } from "#shared/types/api"

/**
 * Versi PORTAL dari Generate Kartu staff -- LOGIC & UI SAMA PERSIS,
 * cuma path navigasi disesuaikan ke rute portal. Endpoint backend yang
 * dipanggil SAMA (permission-nya SUDAH diperluas terima izin portal
 * can_view_idcard). CATATAN: versi ini BELUM menyertakan fitur Retouch
 * Foto -- source Next.js yang jadi acuan porting ini memang belum
 * punya fitur itu di versi portal (fitur Retouch baru ditambahkan
 * belakangan, ke versi STAFF saja).
 */
...
<template>
  <div v-if="result">
    <PageHeader title="Generate Kartu">
    ...
    <Card class="flex flex-col items-center gap-4 p-8 text-center">
      ...
      <div class="flex gap-2">
        <Button variant="outline" as-child><NuxtLink :to="`/portal/idcard-cards/${result.id}`">Lihat Detail</NuxtLink></Button>
      ...
      <Card class="space-y-4 p-4">
        ...
        <div v-else class="space-y-1.5">
          ...
          <p v-if="holders.length === 0" class="text-xs text-muted-foreground">
            Belum ada data. Tambahkan dulu lewat halaman <NuxtLink to="/portal/idcard-holders" class="text-primary hover:underline">Data Visitor/BHL</NuxtLink>.
          </p>
