...
/**
 * Versi Portal dari Mail Server Users -- SAMA endpoint dgn staff,
 * TANPA AddZentyalUserDialog & tombol Hapus (SENGAJA -- tambah/hapus
 * user Zentyal TETAP staff-only, tidak diperluas ke izin portal
 * can_view_zentyal_users).
 */
const PAGE_SIZE = 20
...
<template>
  <div>
    <PageHeader title="Mail Server - Users">
          ...
          <TableRow v-for="user in data?.results" :key="user.dn" v-else>
            ...
            <TableCell>
              <div class="flex justify-end">
                <ToggleUserStatusButton source="zentyal" :user-dn="user.dn" :user-label="user.display_name || user.username" :is-enabled="user.is_enabled ?? true" />
                <DirectoryResetPasswordButton source="zentyal" :user-dn="user.dn" :user-label="user.display_name || user.username" />
              </div>
            </TableCell>
          </TableRow>
        </TableBody>
