...
/**
 * AddCloudflareRecordButton DIPAKAI ULANG APA ADANYA dari staff --
 * tambah record TERMASUK cakupan portal (add+edit, TANPA hapus).
 * Aksi per-baris pakai PortalCloudflareRecordActionsMenu (cuma Edit).
 */
const PAGE_SIZE = 20
...
const { data, pending, error } = await useAsyncData(
  () => `portal-cloudflare-records-${zoneId.value}-${query.value}`,
...
<template>
  <div>
    <PageHeader title="Cloudflare - DNS Records">
      <template #description>
        <NuxtLink to="/portal/cloudflare/zones" class="inline-flex items-center gap-1 text-primary hover:underline">
          <ArrowLeft class="h-3 w-3" /> Kembali ke Daftar Domain
        </NuxtLink>
      </template>
      <template #action>
        <AddCloudflareRecordButton :zone-id="zoneId" />
      </template>
    </PageHeader>
    <Card>
      ...
      <Table v-else>
        ...
        <TableBody>
          ...
          <TableRow v-for="record in data?.results" :key="record.id" v-else>
            ...
            <TableCell><PortalCloudflareRecordActionsMenu :zone-id="zoneId" :record="record" /></TableCell>
          </TableRow>
        </TableBody>
      </Table>
