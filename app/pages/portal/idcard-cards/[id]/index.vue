...
/**
 * ChangeStatusButton (staff) DIPAKAI ULANG APA ADANYA -- generik,
 * sudah auto-terdaftar global. TANPA DeleteCardButton (SENGAJA --
 * hapus kartu PERMANEN, termasuk file fisik, TETAP staff-only).
 */
...
const { data: card } = await useAsyncData(
  () => `portal-idcard-card-detail-${cardId.value}`,
  ...
  <div v-if="card">
    <PageHeader :title="card.holder_name">
      <template #description>
        <NuxtLink to="/portal/idcard-cards" class="inline-flex items-center gap-1 text-primary hover:underline">
