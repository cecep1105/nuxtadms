...
const { data, pending, error } = await useAsyncData(
  () => `ad-dns-records-${zoneDn.value}-${query.value}`,
...
<template>
  <div>
    <PageHeader title="NetMgmt / Active Directory / DNS Records">
      <template #description>
        <div class="flex flex-col gap-1">
          <NuxtLink to="/netmgmt/active-directory/dns" class="inline-flex items-center gap-1 text-primary hover:underline">
            <ArrowLeft class="h-3 w-3" /> Kembali ke Daftar Zone
          </NuxtLink>
          <span class="font-mono text-[11px] text-muted-foreground">{{ zoneDn }}</span>
        </div>
      </template>
      <template #action><AddRecordButton :zone-dn="zoneDn" /></template>
    </PageHeader>
    <Card>
      ...
      <Table v-else>
        ...
        <TableBody>
          <TableRow v-if="!data?.results.length">
            <TableCell :colspan="5" class="py-8 text-center text-muted-foreground">Tidak ada record ditemukan.</TableCell>
          </TableRow>
          <TableRow v-for="record in data?.results" :key="record.raw_b64" v-else>
            ...
            <TableCell><RecordActionsMenu :zone-dn="zoneDn" :record="record" /></TableCell>
          </TableRow>
        </TableBody>
      </Table>
