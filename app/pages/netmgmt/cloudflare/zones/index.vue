...
<template>
  <div>
    <PageHeader title="NetMgmt / Cloudflare / Domains" :description="`Domain (zone) yang bisa diakses token API ini (${data?.count ?? 0} domain).`" />
    <Card>
          ...
          <TableRow v-for="zone in data?.results" :key="zone.id" v-else>
            ...
            <TableCell>
              <div class="flex justify-end">
                <NuxtLink :to="`/netmgmt/cloudflare/zones/${zone.id}`" class="inline-flex items-center gap-1.5 text-xs font-medium text-primary hover:underline">
