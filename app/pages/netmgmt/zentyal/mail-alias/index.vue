...
const { data, pending, error } = await useAsyncData(
  "zentyal-mail-alias",
  () => request<Paginated<MailPostfixAlias>>(`/netmgmt/zentyal/postfix-alias/mailalias/?${query.value}`),
  { watch: [query] }
)
</script>
...
<template>
  <div>
    <PageHeader title="NetMgmt / Zentyal / Mail-Alias" description="Daftar Email Alias">
      ...
      <Table v-else>
        <TableHeader>
          <TableRow>
            ...
            <TableHead><RouterOSSortableHeader column-key="mail" label="Mail Alias" /></TableHead>
            <TableHead><RouterOSSortableHeader column-key="maidrop" label="Mail Member" /></TableHead>
        ...
        <TableBody>
          ...
          <TableRow v-for="alias in data?.results" :key="alias.uid" v-else>
            <TableCell class="font-mono">{{ alias.uid }}</TableCell>
            <TableCell class="font-medium">{{ alias.mail }}</TableCell>
            <TableCell>
              <DropdownMenu>
                <DropdownMenuTrigger as-child>
                  ...
                  >
                    {{ alias.maildrop.length }} item
                  </button>
                </DropdownMenuTrigger>

                <DropdownMenuContent align="start">
                  <DropdownMenuItem
                    v-for="tag in alias.maildrop"
                    :key="tag"
                  >
                    {{ tag }}
                  </DropdownMenuItem>
