...
const props = defineProps<{ item: MikrotikNetwatchItem; basePath: string }>()
...
const codeMatch = computed(() => props.item.comment?.match(/(SID|CRT|CID|SN):\s*[^|]+?(?=\s*\||$)/)?.[0] || " ")
</script>
...
<template>
  <div :class="cn('rounded-lg border p-4 transition-colors', isDown ? 'border-destructive/40 bg-destructive/5' : 'border-border bg-card')">
    ...
    <div class="mt-3 flex justify-between border-t border-border pt-2">
      <span class="text-xs">{{ codeMatch }}</span>
